...
from src.common.data import load_data, data_wrapper, transforms
...
def update(model, x, y, optimizer, criterion, metric, stats, grad_clip,
...
def evaluate(model, dataloader, criterion, metric, stats, batch_size, seed,
...
def run(epochs, run_mode, batch_size, grad_clip, seed=None, **kwargs):
    ...
    camera_lambda = kwargs['camera_lambda']
    ...
    for e in range(epochs):
        ...
        for dl in train_dataloaders:
            for states, actions, cameras, values in tqdm(dl(1, batch_size, seed, sequence_len)):
                ...
                update(model=model, x=states, y=(actions, cameras, values),
