...
class CnnLstmNet(nn.Module):
    ...
    def forward(self, x):
        ...
        h = self.lstm(h, (h0.detach(), c0.detach()))[0]
        return h
...
class InventoryNet(nn.Module):
    ...
    def __init__(self):
        ...
        self.linear = nn.Sequential(
            nn.Linear(self.config.settings.model.lstm_hidden_dim, self.config.settings.model.inventory_hidden_dim),
            DynLayerNorm(),
            nn.ReLU()
        )
        self.value = nn.Linear(self.config.settings.model.inventory_hidden_dim,
                               self.config.settings.model.inventory_final_dim)
...
class Net(nn.Module):
    ...
    def imitate(self, state):
        ...
        inventory_probs = torch.softmax(inventory, dim=-1)
        inventory_dist = Categorical(probs=inventory_probs)
        inventory_action = inventory_dist.sample()
