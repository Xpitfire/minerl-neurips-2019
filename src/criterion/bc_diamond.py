...
class BehaviouralCloningLoss(torch.nn.Module):
    ...
    def forward(self, pred, target):
        ...
        assert self.config.settings.model.actor_craft_final_dim == pred['craft_net_out'].shape[-1]
        assert self.config.settings.model.actor_equip_final_dim == pred['equip_net_out'].shape[-1]
        assert self.config.settings.model.actor_place_final_dim == pred['place_net_out'].shape[-1]
        assert self.config.settings.model.actor_nearbyCraft_final_dim == pred['nearbyCraft_net_out'].shape[-1]
        assert self.config.settings.model.actor_nearbySmelt_final_dim == pred['nearbySmelt_net_out'].shape[-1]
