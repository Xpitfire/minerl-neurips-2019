...
class BehaviouralCloningLoss(torch.nn.Module):
    ...
    def forward(self, pred, target):
        ...
        assert self.config.settings.model.inventory_final_dim == pred['inventory_logits'].shape[-1]
        ...
        inventory = target['inventory'].long()
        inventory_loss = self.ce(pred['inventory_logits'], inventory)
        # log rescaling trick
        value_loss = torch.log(1 + self.mse(pred['value'], target['values']))
        loss = camera_loss + move_loss + craft_loss + equip_loss + place_loss + nearbyCraft_loss + nearbySmelt_loss + value_loss + inventory_loss
        ...
        self.collectible.update(category='train', **{'inventory_loss': inventory_loss.detach().cpu().item()})
        return loss
