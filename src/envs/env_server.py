...
class Handler:
    ...
    def handle_render(self, client, data):
        env = self.env_pool[self.env_register[client]]
        try:
            return env.render(data)
        except Exception as e:
            logging.error("EXCEPTION DURING env.render, not supported...\n{}".format(e))
    ...
    def _make_env(env_name):
        ...
        env = gym_sync_create(env_name, threading.get_ident())
        env.reset()
        return env
...
def parse_args():
    ...
    parser.add_argument("--env", choices=["MineRLTreechop-v0", "MineRLObtainDiamond-v0", "MineRLObtainDiamondDense-v0"],
                        default="MineRLObtainDiamond-v0", help="Environment name")
